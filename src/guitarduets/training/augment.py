...
class OppositePanning(nn.Module):
    ...
    def forward(self, wav):
        ...
        if self.training and random.random() < self.p and channels > 1:
            ...
            offset = th.empty(batch, sources // 2, 1, 1, device=device).uniform_(*self.offset_range)
            pan_factors_opposite = th.clamp(pan_factors + offset, -1, 1)
            pan_factors = th.stack([pan_factors, pan_factors_opposite], dim=2).view(batch, sources, 1, 1)
