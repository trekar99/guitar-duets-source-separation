...
def downsample_binary_tensor(tensor, output_length):
    ...
    indices = (torch.arange(output_length, device=tensor.device) * ratio).long()
    downsampled_tensor = torch.gather(
        tensor,
        -1,
        indices.expand(tensor.size(0), tensor.size(1), output_length),
    )
    return (downsampled_tensor >= 0.5).to(torch.uint8)
...
class HTDemucs(nn.Module):
    ...
    def forward(self, mix, notes=None):
        ...
        if self.use_train_segment:
            if self.training:
                self.segment = self.segment #Fraction(mix.shape[-1], self.samplerate)
            else:
                training_length = int(self.segment * self.samplerate)
